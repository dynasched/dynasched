...
When building DynaSched from its repository sources, the GNU Autotools
...
.. note:: The GNU Autotools are *not* required when building DynaSched
          from distribution tarballs.  DynaSched distribution tarballs
...
Python >= |python_min_version| is required when building DynaSched
from a Git clone for generating the "show help" messages and
building the DynaSched documentation and man pages.

Generating the show help messages can be accomplished with core
Python; only building the full DynaSched documentation and man pages
requires additional Python packages (:ref:`see below <developers-requirements-sphinx-label>`).

Perl
----

DynaSched still uses Perl for a few of its build scripts (most notably,
``autogen.pl``).

Generally speaking, any recent-ish release of Perl 5 should be
sufficient to correctly execute DynaSched's Perl scripts.
...
all flavors of lex and doing more interesting work on DynaSched, we
greatly prefer the latter.

Note that no testing has been performed to see what the minimum
version of Flex is required by DynaSched.  We suggest that you use
v2.5.35 at the earliest.

For now, DynaSched will allow developer builds with Flex |flex_min_version|.  This
is primarily motivated by the fact that RedHat/CentOS 5 ships with
Flex 2.5.4.  It is likely that someday DynaSched developer builds will
...
Official DynaSched distribution tarballs contain pre-built HTML
...
However, the HTML documentation and man pages are *not* stored in DynaSched's
...
DynaSched from a Git clone, you will need Sphinx (and some Python
...
.. note:: If you have built/installed DynaSched from a Git clone and
