...
Installing and running Sphinx (building the DynaSched docs)
===========================================================

As with all content in the Developer's Guide, this section is only
relevant for developers who work in the DynaSched code base itself.
End users who install a binary DynaSched package or build from an
official DynaSched distribution tarball do not need to have Sphinx
installed.
...
The preferred method of installing Sphinx for DynaSched documentation
...
other Python modules in the ``dsched-docs-venv`` tree under your DynaSched
Git clone directory:

.. code-block:: sh

   # Create the Python virtual environment
   shell$ cd TOP_OF_DynaSched_GIT_CLONE
   shell$ python3 -m venv dsched-docs-venv
   # Or: python3 -m virtualenv dsched-docs-venv
   # Or: virtualenv --python=python3 dsched-docs-venv

   # Activate the virtual environment
   shell$ . ./dsched-docs-venv/bin/activate

   # Notice that the shell prompt changes
   # Now install the required Python modules
   (dsched-docs-venv) shell$ pip3 install -r docs/requirements.txt
...
the DynaSched documentation in a system-wide location.
...
DynaSched's build environment is setup to invoke Sphinx automatically;
...
rest of the DynaSched software.  This can be a huge time-saver when
...
.. note:: The fully-built HTML and man page docs are included in
          official DynaSched distribution tarballs.  Meaning: if you
          download an official distribution DynaSched tarball,
          the pre-built HTML and man page files are included
          in the tarball.

          Sphinx is a requirement for *developers* who want to build
          the DynaSched docs.  End users do *not* need to have Sphinx
          available to build DynaSched or have its docs installed from
...
is a nice time saver for DynaSched (e.g., if you are iterating over
...
#. The HTML docs are installed (by default) to
   ``$prefix/share/doc/dsched/html``.  You can use a web browser to
   ...
   .. code:: sh
      ...
      shell$ open $prefix/share/doc/dsched/html/index.html
