...
* RST: Have a single line of text, underlined by non-ASCII
  ...
  * Which non-ASCII character is used for the underlying does not
    ...
    In these DynaSched docs, the sequence of underline characters we use
    are:
...
* RST: Use the ``:doc:`` directive.
  ...
  The page path is relative to the ``docs`` dir in the DynaSched git tree.

Macros
------

* MD: There are no macros in Markdown.
* RST: We have defined a few DynaSched-specific macros in RST.  You can
  insert these macros anywhere in RST content text.

  * ``|dsched_ver|`` is the full DynaSched version number, including
    alpha/beta/rc/greek denotation.  For example ``5.0.0rc1``.

  * ``|dsched_series|`` is the major/minor DynaSched version, e.g.,
    ``5.0.x``.

    .. important:: Think twice about hard-coding the DynaSched version
