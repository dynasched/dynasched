...
DynaSched requires the following support libraries with the minimum listed versions:
...
.. list-table::
   ...
   * - `Hardware Locality <https://www.open-mpi.org/projects/hwloc/>`_
     - |hwloc_min_version|
     - This library is required; DynaSched will not build without it.
   * - `Libevent <https://libevent.org/>`_
     - |event_min_version|
     - This library is required; DynaSched will not build without it.
   * - `PMIx <https://github.com/openpmix/openpmix>`_
     - |pmix_min_version|
     - This library is required; DynaSched will not build without it.

These support libraries are fundamental to DynaSched's operation
...
* Loading the DynaSched shared library causes the loading of Libevent
...
.. important:: If possible, use your OS / environment's package
   ...
   invoking DynaSched's ``configure`` script.
...
example |mdash| only one of Libevent, Hwloc and PMIx, that somewhat simplifies the
final DynaSched configuration, and therefore avoids some potentially
erroneous configurations.

Overriding ``configure`` behavior
---------------------------------

DynaSched's ``configure`` will (minus any configure flags) attempt to
find the HWLOC and libevent packages in standard locations.
If a required package is not found and no flags
...
<label-building-dsched-cli-options-required-support-libraries>`.
...
.. danger:: Be very, very careful when overriding ``configure``'s
   ...
   requirement: that Dynasched infrastructure will load
   *exactly one copy* of each support library.  For simplicity, it may
   ...
   DynaSched was compiled and built against.

   For example, using the DynaSched installed from the sample
   ``configure`` line (above), you may want to prefix your run-time
   linker search path (e.g., ``LD_LIBRARY_PATH`` on Linux) with
   ``$HOME/dsched-install/lib:/opt/libevent/lib:/opt/hwloc/lib``.
   This will ensure that the linker finds
   the support libraries used by your DynaSched installation tree,
