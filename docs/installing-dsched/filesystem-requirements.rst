...
.. warning:: If you are building DynaSched on a network filesystem, the
   machine you on which you are building *must* be time-synchronized
   with the file server.

Specifically: DynaSched's build system *requires* accurate filesystem
...
In this case, you should remove the DynaSched source directory and
start over (e.g., by re-extracting the DynaSched tarball): either
