...
# Minimum versions
my $dsched_automake_version = "1.13.4";
my $dsched_autoconf_version = "2.69";
my $dsched_libtool_version = "2.4.2";

# Search paths
my $dsched_autoconf_search = "autoconf";
my $dsched_automake_search = "automake";
my $dsched_libtoolize_search = "libtoolize;glibtoolize";
...
sub ignored {
    ...
    # If this directory does not have .dsched_ignore, or if it has a
    # .dsched_unignore that has my username in it, then add it to the
    # list of components.
    my $ignored = 0;

    if (-f "$dir/.dsched_ignore") {
        $ignored = 1;
    }
    if (-f "$dir/.dsched_unignore") {
        open(UNIGNORE, "$dir/.dsched_unignore") ||
            my_die "Can't open $dir/.dsched_unignore file";
        my $unignore;
        $unignore .= $_
            while (<UNIGNORE>);
        close(UNIGNORE);
    ...
    if (exists($exclude_list->{$framework}) &&
        ...
        if (-d $dir) {
            ...
            foreach my $d (sort(readdir(DIR))) {
                ...
                verbose "--- Found DynaSched / $framework / $d component: src/mca/$framework/$d\n";
                ...
                # Check ignore status
                if (ignored("$dir/$d")) {
                    verbose "    => Ignored (found .dsched_ignore file)\n";
                } else {
...
sub mca_generate_framework_header(\$\@) {
    ...
    foreach my $framework (@frameworks) {
        ...
        if ($framework ne "common" and $framework ne "src") {
            my $framework_name = "dsched_${framework}_base_framework";
    ...
    my $ifdef_string = uc "dsched_FRAMEWORKS_H";
    open(FRAMEWORKS_OUT, ">src/include/dsched_frameworks.h");
...
DSCHED_EXPORT extern pmix_mca_base_framework_t *dsched_frameworks[];
DSCHED_EXPORT extern char *dsched_framework_names[];

#endif /* $ifdef_string */\n\n";
    close(FRAMEWORKS_OUT);

    open(FRAMEWORKS_OUT, ">src/include/dsched_frameworks.c");
    printf FRAMEWORKS_OUT "%s", "/*
 * This file is autogenerated by autogen.pl. Do not edit this file by hand.
 */
...
pmix_mca_base_framework_t *dsched_frameworks[] = {
$framework_array_output    NULL
};

char *dsched_framework_names[] = {
...
sub mca_process_project {
    ...
    foreach my $d (@my_dirs) {
        ...
        if ("common" eq $d ||
            ...
            verbose "\n=== Found DynaSched framework: src/mca/$d\n";
...
dnl Frameworks in the DynaSched project and their corresponding directories
m4_define([mca_dsched_framework_list], [$frameworks_comma])
...
";
    ...
    foreach my $f (@mykeys) {
        ...
        $m4 .= "dnl Components in the dsched / $f framework
m4_define([mca_dsched_${f}_m4_config_component_list], [$m4_config_component_list])
m4_define([mca_dsched_${f}_no_config_component_list], [$no_config_component_list])
...
tools:
    ...
    GNU Autoconf: $dsched_autoconf_version
    GNU Automake: $dsched_automake_version
    GNU Libtool: $dsched_libtool_version
=================================================================\n";
    my_exit(1);
}
...
sub patch_autotools_output {
    ...
    my $replace_string = 'darwin*)
      # DynaSched patched for Darwin / MacOS Big Sur.  See
...
sub export_version {
    ...
    $m4 .= "m4_define([DSCHED_${name}_MIN_VERSION], [$version])\n";
    $m4 .= "m4_define([DSCHED_${name}_NUMERIC_MIN_VERSION], [$hex])\n";
}
...
sub export_max_version {
    ...
    my @version_splits = split(/\./,$version);
    my $hex = sprintf("0x%04x%02x%02x", $version_splits[0], $version_splits[1], $version_splits[2]);
    $m4 .= "m4_define([DSCHED_${name}_MAX_VERSION], [$version])\n";
    $m4 .= "m4_define([DSCHED_${name}_NUMERIC_MAX_VERSION], [$hex])\n";
}
...
sub get_and_define_min_versions() {
    ...
    while (<IN>) {
          ...
          if ($fields[0] eq "automake_min_version") {
              if ($fields[1] ne "\n") {
                  $dsched_automake_version = $fields[1];
              }
          }
          elsif($fields[0] eq "autoconf_min_version") {
              if ($fields[1] ne "\n") {
                  $dsched_autoconf_version = $fields[1];
              }
          }
          elsif($fields[0] eq "libtool_min_version") {
              if ($fields[1] ne "\n") {
                  $dsched_libtool_version = $fields[1];
              }
          }
...
my $project_name_long = "DynaSched";
my $project_name_short = "dsched";
...
# Verify that we're in the DynaSched root directory by checking for a token file.

my_die "Not at the root directory of a DynaSched source tree"
    if (! -f "config/dsched_mca.m4");

$force_arg = 1;

my_die "autogen.pl has been invoked in the source tree of a DynaSched distribution tarball; aborting...
...
&find_and_check("autoconf", $dsched_autoconf_search, $dsched_autoconf_version);
&find_and_check("libtool", $dsched_libtoolize_search, $dsched_libtool_version);
&find_and_check("automake", $dsched_automake_search, $dsched_automake_version);
...
# Figure out if we're at the top level of the DynaSched tree or not.
if (! (-f "VERSION" && -f "configure.ac" && -f $topdir_file)) {
    print("\n\nYou must run this script from the top-level directory of the DynaSched tree.\n\n");
    my_exit(1);
}
...
verbose "
================================================
DynaSched autogen: completed successfully.  w00t!
================================================\n\n";
