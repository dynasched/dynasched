...
%{!?shell_scripts_basename: %define shell_scripts_basename dschedvars}
...
# installed lots of other stuff in the BUILD_ROOT before DynaSched).
# type: bool (0/1)
...
# On some platforms, DynaSched just flat-out doesn't work with
...
# Note that the name "dsched" is hard-coded in
# src/mca/dinstalldirs/config for pkgdatadir; there is currently no
# easy way to have DynaSched change this directory name internally.  So we
# just hard-code that name here as well (regardless of the value of
# %{name} or %{_name}).
%define _pkgdatadir /opt/%{name}/%{version}/share/dsched
...
%define modulefile_path /opt/%{name}/%{version}/share/dschedmodulefiles
%endif
...
%{!?_pkgdatadir: %define _pkgdatadir %{_datadir}/dsched}
...
Summary: A Dynamic Scheduler
Name: %{?_name:%{_name}}%{!?_name:dsched}
Version: $VERSION
Release: 1%{?dist}
License: BSD
Group: Development/Libraries
Source0: https://github.com/dynasched/dynasched/releases/download/v%{version}/dsched-%{version}.tar.bz2
Packager: %{?_packager:%{_packager}}%{!?_packager:%{_vendor}}
Vendor: %{?_vendorinfo:%{_vendorinfo}}%{!?_vendorinfo:%{_vendor}}
Distribution: %{?_distribution:%{_distribution}}%{!?_distribution:%{_vendor}}
Prefix: %{_prefix}
Provides: dsched
Provides: dsched = %{version}
...
%description
Static schedulers have been the norm for many years. These systems
calculate a schedule based on the current known task list, and then
execute that schedule without changes. If new tasks are added, they
are scheduled using a "backfill" algorithm as currently executing
jobs complete.

In contrast, dynamic schedulers recompute their schedule whenever new
tasks are submitted. This can include preempting executing jobs and
reassigning their resources to new tasks.

DynaSched provides a framework for developing, testing, and ultimately
fielding a range of dynamic schedulers for research and production
purposes.

This RPM contains all the tools necessary to compile DynaSched.
%endif
