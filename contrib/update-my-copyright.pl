...
# in the tops of DynaSched source files before committing back to
# the repository.  Set the environment variable
# DSCHED_COPYRIGHT_SEARCH_NAME to a short (case-insensitive) name that
# indicates your copyright line (e.g., "cisco"), and set the env
# variable DSCHED_COPYRIGHT_FORMAL_NAME with your organization's formal
...
$my_search_name = $ENV{DSCHED_COPYRIGHT_SEARCH_NAME}
    if (defined($ENV{DSCHED_COPYRIGHT_SEARCH_NAME}));
$my_formal_name = $ENV{DSCHED_COPYRIGHT_FORMAL_NAME}
    if (defined($ENV{DSCHED_COPYRIGHT_FORMAL_NAME}));
