...
specfile="dsched.spec"
prefix=${prefix:-"/opt/dsched"}
...
unset first
echo "--> Found DynaSched version: $version"
...
specdest="$rpmtopdir/SPECS/dsched-$version.spec"
...
if test "$build_srpm" = "yes"; then
    echo "--> Building the DynaSched SRPM"
...
if test "$build_single" = "yes"; then
    echo "--> Building the single DynaSched RPM"
...
if test "$build_multiple" = "yes"; then
    echo "--> Building the multiple DynaSched RPM"
...
------------------------------------------------------------------------------
====                FINISHED BUILDING DynaSched RPM                       ====
------------------------------------------------------------------------------
