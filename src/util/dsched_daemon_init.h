...
#ifndef DSCHED_DAEMON_INIT_H
#define DSCHED_DAEMON_INIT_H
 ...
 *
 * @retval DSCHED_SUCCESS Indicates that the conversion was successful
 * @retval DSCHED_ERROR Indicates that the conversion was not successful - a fork could not be
 * completed.
 */
DSCHED_EXPORT int dsched_daemon_init_callback(char *working_dir, int (*parent_fn)(pid_t child));

END_C_DECLS

static inline int dsched_daemon_init(char *working_dir)
{
    return dsched_daemon_init_callback(working_dir, NULL);
}

END_C_DECLS

#endif /* DSCHED_DAEMON_INIT_H */
