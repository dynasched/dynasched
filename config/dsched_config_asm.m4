...
AC_DEFUN([DSCHED_CHECK_GCC_ATOMIC_BUILTINS], [
  if test -z "$dsched_cv_have___atomic" ; then
...
__atomic_add_fetch(&tmp64, 1, __ATOMIC_RELAXED);]])],
        [dsched_cv_have___atomic=yes],
        [dsched_cv_have___atomic=no])
    AC_MSG_RESULT([$dsched_cv_have___atomic])

    if test "$dsched_cv_have___atomic" = "yes" ; then
...
__atomic_compare_exchange_n(&tmp64, &old64, 1, 0, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
__atomic_add_fetch(&tmp64, 1, __ATOMIC_RELAXED);]])],
            [dsched_cv_have___atomic_64=yes],
            [dsched_cv_have___atomic_64=no])
      AC_MSG_RESULT([$dsched_cv_have___atomic_64])
      if test "$dsched_cv_have___atomic_64" = "yes" ; then
...
dnl #################################################################
dnl
dnl DSCHED_CONFIG_ASM()
dnl
dnl DEFINE DSCHED_ATOMIC_C11 if C11 atomics should be used
dnl DEFINE DSCHED_ATOMIC_GCC_BUILTIN if gcc builtin atomics should be used
dnl DEFINE DSCHED_ATOMIC_X86_64 if we're on an x86_64 platform
dnl
dnl #################################################################
AC_DEFUN([DSCHED_CONFIG_ASM],[
    AC_REQUIRE([DSCHED_SETUP_CC])
    ...
    dsched_atomic_c11=0
    dsched_atomic_gcc_builtin=0

    DSCHED_CHECK_GCC_ATOMIC_BUILTINS
    if test "x$enable_c11_atomics" != "xno" && test "$dsched_cv_c11_supported" = "yes" ; then
        dsched_atomic_style="c11"
        dsched_atomic_c11=1
    elif test "x$enable_c11_atomics" = "xyes"; then
        AC_MSG_WARN([C11 atomics were requested but are not supported])
        AC_MSG_ERROR([Cannot continue])
    elif test "$enable_builtin_atomics" != "no" && test "$dsched_cv_have___atomic" = "yes" ; then
        dsched_atomic_style="gcc"
        dsched_atomic_gcc_builtin=1
    ...
    x86_64-*x32|i?86-*|x86_64*|amd64*)
        AC_MSG_RESULT([yes])
        AC_DEFINE([DSCHED_ATOMIC_X86_64], [1], [whether building on x86_64 platform])
        ;;
    *)
        AC_MSG_RESULT([no])
        ;;
    ...
    AC_MSG_RESULT([$dsched_atomic_style])

    AC_DEFINE_UNQUOTED([DSCHED_ATOMIC_C11], [$dsched_atomic_c11],
        [Use C11 style atomics])

    AC_DEFINE_UNQUOTED([DSCHED_ATOMIC_GCC_BUILTIN], [$dsched_atomic_gcc_builtin],
