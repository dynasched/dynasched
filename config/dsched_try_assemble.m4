...
dnl DSCHED_TRY_ASSEMBLE(asm-code, [action-if-success], [action-if-fail])
dnl
...
AC_DEFUN([DSCHED_TRY_ASSEMBLE],
[cat >conftest.s <<EOF
[$1]
EOF
if test "$CC" = "$CCAS" ; then
    dsched_assemble="$CCAS $CCASFLAGS -c conftest.s >conftest.out 2>&1"
else
    dsched_assemble="$CCAS $CCASFLAGS -o conftest.o conftest.s >conftest.out 2>&1"
fi
if AC_TRY_EVAL(dsched_assemble); then
  # save the warnings
