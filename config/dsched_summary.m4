...
dnl DSCHED_SUMMARY_ADD(section, topic, unused, result)
dnl
dnl Turn around and call OAC_SUMMARY_ADD
dnl -----------------------------------------------------------
AC_DEFUN([DSCHED_SUMMARY_ADD],[
    OAC_SUMMARY_ADD([$1], [$2], [$4])
])

dnl DSCHED_SUMMARY_PRINT
dnl
dnl Print a bunch of PRRTE summary configuration information and
dnl then turn around and call OAC_SUMMARY_PRINT.
dnl -----------------------------------------------------------
AC_DEFUN([DSCHED_SUMMARY_PRINT],[
    cat <<EOF >&2

DSCHED configuration:
-----------------------
Version: $DSCHED_MAJOR_VERSION.$DSCHED_MINOR_VERSION.$DSCHED_RELEASE_VERSION$DSCHED_GREEK_VERSION
EOF
    ...
    if test ! -z $with_dsched_platform ; then
        echo "Platform file: $with_dsched_platform" >&2
    else
