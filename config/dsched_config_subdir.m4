...
AC_DEFUN([DSCHED_CONFIG_SUBDIR],[
DSCHED_VAR_SCOPE_PUSH([subdir_parent sub_configure subdir_dir subdir_srcdir subdir_cache_file subdir_args subdir_dots total_dir dir_part temp])
...
if test "$subdir_dir" != ":" && test -d $srcdir/$subdir_dir; then
    AC_MSG_NOTICE([DSCHED configuring in $subdir_dir])
