...
# DSCHED_LOAD_PLATFORM()
# --------------------
AC_DEFUN([DSCHED_LOAD_PLATFORM], [
    AC_ARG_WITH([dsched-platform-patches-dir],
        [AS_HELP_STRING([--with-dsched-platform-patches-dir=DIR],
                        [Location of the platform patches directory. If you use this option, you must also use --with-platform.])])

    AC_ARG_WITH([dsched-platform],
        [AS_HELP_STRING([--with-dsched-platform=FILE],
    ...
    m4_ifval([autogen_platform_file], [
        if test "$with_dsched_platform" = "" ; then
            with_dsched_platform=autogen_platform_file
        fi])
    if test "$with_dsched_platform" = "yes" ; then
        AC_MSG_ERROR([--with-platform argument must include FILE option])
    elif test "$with_dsched_platform" = "no" ; then
        AC_MSG_ERROR([--without-platform is not a valid argument])
    elif test "$with_dsched_platform" != "" ; then
        # if not an absolute path, check in contrib/platform
        if test ! "`echo $with_dsched_platform | cut -c1`" = "/" && test ! "`echo $with_dsched_platform | cut -c2`" = ".." ; then
            if test -r "${srcdir}/contrib/platform/$with_dsched_platform" ; then
                with_dsched_platform="${srcdir}/contrib/platform/$with_dsched_platform"
            fi
        fi

        # make sure file exists
        if test ! -r "$with_dsched_platform" ; then
            AC_MSG_ERROR([platform file $with_dsched_platform not found])
        fi

        # eval into environment
        DSCHED_LOG_MSG([Loading environment file $with_dsched_platform, with contents below])
        DSCHED_LOG_FILE([$with_dsched_platform])

        # setup by getting full pathname for the platform directories
        platform_base="`dirname $with_dsched_platform`"
        platform_file="`basename $with_dsched_platform`"
        ...
        if test "$DSCHED_PLATFORM_LOADED" != "" ; then
           platform_loaded="$DSCHED_PLATFORM_LOADED"
        else
           platform_loaded="$with_dsched_platform"
        fi
        echo "Loaded platform arguments for $platform_loaded"
        DSCHED_LOG_MSG([Loaded platform arguments for $platform_loaded])
        ...
        if test -r "${platform_file_dir}/${platform_alt_mca_file}" ; then
            AC_SUBST(DSCHED_DEFAULT_MCA_PARAM_CONF, [$platform_file_dir/$platform_alt_mca_file])
            AC_SUBST(DSCHED_PARAM_FROM_PLATFORM, "yes")
        # if not, see if a file is there with the default name
        elif test -r "${platform_file_dir}/dsched-mca-params.conf" ; then
            AC_SUBST(DSCHED_DEFAULT_MCA_PARAM_CONF, [$platform_file_dir/dsched-mca-params.conf])
            AC_SUBST(DSCHED_PARAM_FROM_PLATFORM, "yes")
        # if not, then just use the default
        else
            AC_SUBST(DSCHED_DEFAULT_MCA_PARAM_CONF, [dsched-mca-params.conf])
            AC_SUBST(DSCHED_PARAM_FROM_PLATFORM, "no")
        fi

        # look where platform file is located for platform.config name
        if test -r "${platform_file_dir}/${platform_alt_config_file}" ; then
            AC_SUBST(DSCHED_DEFAULT_CONFIG, [$platform_file_dir/$platform_alt_config_file])
            AC_SUBST(DSCHED_PARAM_FROM_PLATFORM, "yes")
        # if not, see if a file is there with the default name
        elif test -r "${platform_file_dir}/dsched.config" ; then
            AC_SUBST(DSCHED_DEFAULT_CONFIG, [$platform_file_dir/dsched.config])
            AC_SUBST(DSCHED_PARAM_FROM_PLATFORM, "yes")
        # if not, then just use the default
        else
            AC_SUBST(DSCHED_DEFAULT_CONFIG, [dsched.config])
            AC_SUBST(DSCHED_PARAM_FROM_PLATFORM, "no")
        fi

        patch_dir="${with_dsched_platform}.patches"
        if test -n "$with_dsched_platform_patches_dir"; then
            if test "$with_dsched_platform_patches_dir" = "yes"; then
                patch_dir="${with_dsched_platform}.patches"
            elif test "$with_dsched_platform_patches_dir" = "no"; then
                AC_MSG_NOTICE([Disabling platform patches on user request])
                patch_dir=""
            elif test -d "$with_dsched_platform_patches_dir"; then
                patch_dir=$with_dsched_platform_patches_dir
            else
                AC_MSG_ERROR([User provided patches directory: $with_dsched_platform_patches_dir not found])
            fi
        fi
        ...
        if test -d "${patch_dir}"; then
            if test ! -f "${patch_done}"; then
                ...
                if test "$patch_found" = "yes"; then
                    ...
                    # Set custom ident for platform patched DSCHED
